...
// - it is because nestedFunction is inside myFunction, it has access  to information in it's surrounding scope
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    let newarr= []
   let yes= arr.map((i) => {
     return newarr.push((i.animal_name).toLowerCase());
    })
    return newarr;
  }
...
CuboidMaker.prototype.volume = function(){
 let product = (this.length * this.width * this.height)
  return product;
}
...
CuboidMaker.prototype.surfaceArea= function(){
 let product = 2 * ((this.length * this.width) + (this.length * this.height) + (this.width * this.height))
  return product;
}
